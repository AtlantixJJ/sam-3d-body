...
|------------|-------|----------------|------------|
| **init_pose** | `(1, 519)` | Zero-pose in 6D rot + cont | 519 |
| **init_camera** | `(1, 3)` | Zeros `[0, 0, 0]` | 3 |
| **init_pose_hand** | `(1, 519)` | Same (hand decoder) | 519 |
...
|-------|-----------|------------|------------|---------|
| **init_to_token_mhr** | 525 | 1024 | 537,600 | Initial pose+cam+cond → token |
| **prev_to_token_mhr** | 522 | 1024 | 534,528 | Previous pose+cam → token |
| **prompt_to_token** | 1280 | 1024 | 1,310,720 | Prompt embeddings → token |
| **init_to_token_mhr_hand** | 525 | 1024 | 537,600 | Hand decoder version |
| **prev_to_token_mhr_hand** | 522 | 1024 | 534,528 | Hand decoder version |

**Input dimension breakdown**:
```python
init_to_token_mhr:
  npose (519) + ncam (3) + cond_dim (3) = 525

prev_to_token_mhr:
  npose (519) + ncam (3) = 522  # No condition for previous estimate
```
...
init_estimate: (B, 519) # Optional initialization
```
...
proj: FFN
  ...
  Structure:
    ...
    ReLU
    Linear(128 → 519):  66,560 params (bias zero-initialized)

  Total: 197,632 parameters
```

#### Output Dimension Breakdown (npose=519)
